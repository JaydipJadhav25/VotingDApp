...
function GetCandidateList() {
    ...
    //get contract instance
     ...
     const [candidateListList , setCandidateListList] = useState([]);
    ...
    useEffect(()=>{ 
        async function getAllCandidateList(){
            try {
                ...
                setCandidateListList(response);
                toast.success("fetched Successfully!")
            } catch (error) {
                console.log("eror : " , error);
                toast.error("to fetch Candidatelist Error!" + error)
   ...
   0 :  " NotSpecified ",
    ...
    console.log(" lists: " , candidateListList);
  ...
  return (
   <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center justify-between">
          ...
          <div className="bg-amber-100 text-amber-800 px-4 py-2 rounded-lg text-sm font-semibold shadow-sm border border-amber-200">
            Total Registered: {candidateListList?.length || 0}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {/* Map through list */}
        {candidateListList?.map((ele, index) => (
          <div 
            key={index} 
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 border-b border-gray-100 pb-4">
                ...
                <div>
                 ...
                 <div>
                  <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold">party</p>
                  <p className="text-sm font-medium text-gray-700 capitalize mt-1">{ele.party}</p>
                </div>

                    <div>
                  <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold">Votes</p>
                  <p className="text-sm font-medium text-gray-700 capitalize mt-1">{ele.votes}</p>
                </div>
              </div>
      ...
      {(!candidateListList || candidateListList.length === 0) && (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <h3 className="text-lg font-medium text-gray-900">No Candidate found</h3>
          <p className="text-gray-500">Get started by registering a new Candidate.</p>
        </div>
      )}
    </div>
  )
